...
"""
bitcoinDF = pd.read_csv("C:/Users/Can/Desktop/myHugeProject/data/BTC_USD Bitfinex Historical Data.csv")
eur_usdDF = pd.read_csv("C:/Users/Can/Desktop/myHugeProject/data/EUR_USD Historical Data.csv")

print(bitcoinDF.head(10))

print("bitcoin length size:",len(bitcoinDF))
print("eur_usd length size:",len(eur_usdDF))

print("not the same length because bitcoin always works.")

merged_df = pd.merge(bitcoinDF, eur_usdDF, on='Date')

print(merged_df.head(10))
print(len(merged_df))

print(type(merged_df["Price_x"][1]))



def remove_comma_and_convert(value):
    # Virgülü kaldır ve noktayı virgül ile değiştir
    value = value.replace(',', '')
    # Değeri float olarak döndür
    return float(value)

# 'Price_x' sütununu dönüştür
merged_df["Price_x"] = merged_df["Price_x"].apply(remove_comma_and_convert)

merged_df['Price_x'] = merged_df['Price_x'].astype(float)

merged_df['Price_y'] = merged_df['Price_y'].astype(float)

print(merged_df.head(10))

print(shapiro(merged_df["Price_x"]))
print(shapiro(merged_df["Price_y"]))

res = stats.spearmanr(merged_df["Price_y"], merged_df["Price_x"])
print(res.statistic)
